...
const isNumber = require('../../../../../helpers/3p/utils/lib/isNumber');
...
describe('is a number', function() {
  ...
  fixtures.forEach(function(num, idx) {
    it(JSON.stringify(num) + ' should be a number', function(done) {
...
describe('is not a number', function() {
  ...
  fixtures.forEach(function(num) {
    it(JSON.stringify(num) + ' should not be a number', function(done) {
      expect(!isNumber(num)).to.equal(true);
      done();
    });
  });
